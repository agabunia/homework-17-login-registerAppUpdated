    ...
    fun login(email: String, password: String) {
        if (email.isBlank() || password.isBlank()) {
            ...
            viewModelScope.launch {
                try {
                    val response: Response<LoginResponse> =LoginNetworkModel.login().login(UserLogin(email, password))
                    if (response.isSuccessful) {
                        _userFlow.value = Resource.Success(data = LoginResponse(response.body().toString()))
